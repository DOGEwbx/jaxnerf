...
class LLFFVideo(Dataset):
  ...
  def _load_renderings(self, args):
    ...
    if not utils.file_exists(imgdir):
      raise ValueError("Image folder {} doesn't exist.".format(imgdir))
    images = []
    for cam in sorted(utils.listdir(imgdir)):
      imgs_percam = []
      # imgfiles_percam = [
      #     path.join(imgdir, cam, f)
      #     for f in sorted(utils.listdir(path.join(imgdir, cam)))
      #     if f.endswith("JPG") or f.endswith("jpg") or f.endswith("png")
      # ]
      imgfiles_percam = []
      for idx in range(args.start, args.end,args.step):
        imgfiles_percam.append(path.join(imgdir, cam, "frame_{:05d}.png".format(idx)))

      for imgfile in imgfiles_percam:
        with utils.open_file(imgfile, "rb") as imgin:
          image = np.array(Image.open(imgin), dtype=np.float32) / 255.
          imgs_percam.append(image)
      imgs_percam = np.stack(imgs_percam,axis=-1)
      images.append(imgs_percam)
    images = np.stack(images,axis=-1) # shape: [H,W,C,F,N]
    shape = images.shape
